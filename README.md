...
```
本データは、ホラー映画における登場人物の死亡傾向を職業別に分析することを目的として構築したRDFデータである。中心となるのは「映画」「職業」「職業別統計」という3つの概念であり、映画ごとにどの職業の人物が何人死亡したかを記述する構造になっている。

まず、hm:Movie_Q1454815 は1本の映画を表すインスタンスであり、本例では映画「13日の金曜日」を示している。このノードは rdf:type hm:Movie により映画クラスに属することが示され、hm:title により日本語タイトルが付与されている。また、rdfs:seeAlso によりWikidata上の同一映画エンティティへのリンクを持ち、外部LODとの接続が確保されている。hm:estimatedTotalCharacters には登場人物の総数（ここでは14）が記録されているが、これは正確な統計ではなく分析目的の推定値であるため、hm:countType hm:Estimated により「推定値」であることが明示されている。

映画ノードから hm:hasRoleStats という述語を通じて、複数の職業別統計ノードに接続される。これらのノードは hm:RoleStatistics クラスのインスタンスであり、「映画 × 職業」の組み合わせごとの統計情報を表す。本例では「一般市民」と「学生」の2種類の職業について統計が存在する。

例えば hm:Movie_Q1454815_RoleStats_Civilian は一般市民に関する統計を表し、hm:role hm:Civilian によって対象職業が一般市民であることを示す。hm:deathCount 4 により、この映画内で一般市民が4人死亡したことが記録されている。同様に hm:Movie_Q1454815_RoleStats_Student では、学生の死亡数が5人であることが記述されている。これらの統計値も映画全体の人数と同様に hm:countType hm:Estimated が付与され、推定値であることが示されている。

職業そのものは単なる文字列ではなく、hm:Civilian や hm:Student のような独立したリソースとして定義されている。これらは rdf:type hm:Role により「職業クラスのインスタンス」であることが示され、rdfs:label により「一般市民」「学生」といった人間可読な名称が与えられている。この設計により、同一職業を複数の映画間で共通利用でき、SPARQLによる横断的な統計分析が可能となる。

以上のように、本データモデルは
「映画」→「職業別統計」→「職業」
という階層構造を持ち、各映画における職業ごとの死亡数をLOD形式で整理したものである。これにより、「警官の死亡が多い映画はどれか」「職業別の死亡数合計」「映画間の死亡傾向比較」といった分析が可能になる。一方で、職業ごとの登場人数や生存人数は記録していないため、厳密な生存率ではなく「死亡傾向の分析」を目的としたデータである点に注意が必要である。
