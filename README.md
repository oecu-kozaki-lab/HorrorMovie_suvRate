...
ホラー映画の一覧は、既存のオープンな知識グラフであるWikidataから取得できるが、このデータには登場人物の職業別統計や生死情報は存在しません．
...
* `rdf:type` `hm:Movie`は映画インスタンスに属することが示されている。  
* `hm:title` により日本語タイトルが付与されている。 
* `rdfs:seeAlso`によりWikidata上の同一映画エンティティへのリンクを持ち、外部LODとの接続が確保されている。  
