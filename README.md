...
ホラー映画の基礎データとして Wikidata（WD） を利用し、映画タイトルを収集した。
...
まず、hm:Movie_Q1454815 は1本の映画を表すインスタンスであり、本例では映画「13日の金曜日」を示している。このノードは rdf:type hm:Movie により映画クラスに属することが示され、hm:title により日本語タイトルが付与されている。また、rdfs:seeAlso によりWikidata上の同一映画エンティティへのリンクを持ち、外部LODとの接続が確保されている。hm:estimatedTotalCharacters には登場人物の総数（ここでは14）が記録されているが、これは正確な統計ではなく分析目的の推定値であるため、hm:countType hm:Estimated により「推定値」であることが明示されている。
 hm:Movie_Q1454815_RoleStats_Civilian は一般市民に関する統計を表し、hm:role hm:Civilian によって対象職業が一般市民であることを示す。hm:deathCount 4 により、この映画内で一般市民が4人死亡したことが記録されている。同様に hm:Movie_Q1454815_RoleStats_Student では、学生の死亡数が5人であることが記述されている。これらの統計値も映画全体の人数と同様に hm:countType hm:Estimated が付与され、推定値であることが示されている。

職業そのものは単なる文字列ではなく、hm:Civilian や hm:Student のような独立したリソースとして定義されている。この設計により、同一職業を複数の映画間で共通利用でき、SPARQLによる横断的な統計分析が可能となる。
